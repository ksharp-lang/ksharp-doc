...
`pub` functions can't be treated as constant value when they are imported. If the function is [cloned](0005-modules.mdx#cloning-functions-from-different-modules) and the function could be treated as constant value, calling the cloned function will be treated as constant value in the module where it was imported. 
